...
resource "aws_iam_policy_attachment" "human_user_policy_attachment" {
  name = "human-upa-${lower(var.environment)}-${lower(random_id.rand.hex)}"
  users = [
...
module "i_am_paulgradie" {
  ...
  name                  = "paul-gradie-${lower(var.environment)}-${lower(random_id.rand.hex)}"
...
module "i_am_adambeck" {
  ...
  name                  = "adam-beck-${lower(var.environment)}-${lower(random_id.rand.hex)}"
  create_iam_access_key = true
  tags                  = local.tags
