...
var checkPermission = function(file: any, mask: number, cb: (arg: string) => void) {
  fs.stat(file, function(error: null, stats: { mode: number }) {
    if (error) {
      cb(error)
    } else {
      cb(parseInt(stats.mode.toString(8), 10).toString())
    }
  });
};
...
class PdfGenerator implements IPdfGenerator {
  ...
  constructor(html: string, pathToPhantom: string, pathToScript: string, paperOptions: any = {}) {
    ...
    logTrace('Setting Filename as ' + this.filename);
    logTrace('pathToScript: ' + this.pathToScript);
    logTrace('Using phantom options: ' + this.phantomArgs);
  ...
  toStream(successCallback: (stream?: ReadStream) => void, errorCallback: PdfErrorCallback) {
    logTrace('toStream called...');
    this.exec((error: Error | null, res?: FileInfo) => {
      if (error) {
        logTrace('toStream execution error. Calling the callback with an error');
      ...
      try {
        logTrace('Executing tostream callback...');
  ...
  toFile(
    ...
    logTrace('toFile called...');
    this.exec((error: Error | null, res?: FileInfo) => {
      if (error) {
        logTrace('toFile execution error.');
        return errorCallback(error);
      }
      if (!res) return;
      ...
      try {
        logTrace('Executing toFile callback (Save to local)');
  ...
  private createStream(res: FileInfo, errorCallback: PdfErrorCallback): ReadStream | void {
    try {
      logTrace('Attempting to create the read stream');
  ...
  private exec(callback: (error: Error | null, res?: FileInfo) => void) {
    logTrace('Execution method inside toStream called...');

    logTrace('Using phantomjs binary: ' + this.pathToPhantom);

    checkPermission(this.pathToPhantom, 1, logTrace);

    fs.access(this.pathToPhantom, fs.constants.X_OK, (err) => {
        console.log(err ? 'cannot execute' : 'can execute');
    });

    const child = spawn(this.pathToPhantom, [...this.phantomArgs, this.pathToScript]);

    logTrace('Child process spawned...');
    ...
    logTrace('Sending script parameters via standard in -- options used: ' + this.paperOptions);
    const config = JSON.stringify({ html: this.html, options: this.paperOptions });

    logTrace('Trying Write to child.stdin...');
    child.stdin.write(config + '\n', 'utf8');

    logTrace('Closing child.stdin...');
    child.stdin.end();
  }
}
