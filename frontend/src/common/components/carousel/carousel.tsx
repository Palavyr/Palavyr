...
interface BorrowedCarouselProps {
    classes: any;
    timeout: number;
}

type BorrowedCarouselState = {
    active: number;
    prevActive: number;
    displayed: number;
};

class BorrowedCarousel extends Component<BorrowedCarouselProps, BorrowedCarouselState> {
    ...
    setActive(index, callback: any = () => {}, runCallbacks = true) {
        ...
        return (
            ...
            >
                {Array.isArray(children) ? (
                    ...
                    <CarouselItem key={`carousel-item0`} display={true} active={true} child={children} animation={animation} timeout={timeout} />
                )}
...
function CarouselItem(props) {
    let swipeHandlers: any = useSwipeable({
...
function Indicators(props) {
    ...
    const IndicatorIcon = props.IndicatorIcon !== undefined ? props.IndicatorIcon : <FiberManualRecordIcon className={classes.indicatorIcon} component={"div"} />;
    let indicators: any = [];
    for (let i = 0; i < props.length; i++) {
        ...
        const style = i === props.active ? Object.assign({}, props.indicatorIconButtonProps.style, props.activeIndicatorIconButtonProps.style) : props.indicatorIconButtonProps.style;
...
export default withStyles(styles)(BorrowedCarousel);
