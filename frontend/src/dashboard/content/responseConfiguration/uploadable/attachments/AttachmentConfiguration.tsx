...
export const AttachmentConfiguration = () => {
    ...
    const removeAttachment = async (fileId: string) => {
        setIsLoading(true);
        const filelinks = await client.Configuration.Attachments.removeAttachment(areaIdentifier, fileId);
        setAttachmentList(filelinks);
        setIsLoading(false);
    };

    const loadAttachments = useCallback(async () => {
        const fileLinks = await client.Configuration.Attachments.fetchAttachmentLinks(areaIdentifier);
        setAttachmentList(fileLinks);
        setLoaded(true);
        setIsLoading(false);
    ...
    const handleFileSave = async (files: File[]) => {
        ...
        if (files.length === 1) {
            ...
            const fileLinks = await client.Configuration.Attachments.saveSingleAttachment(areaIdentifier, formData);
            setAttachmentList(fileLinks);
        } else if (files.length > 1) {
            files.forEach((file: File) => {
                formData.append("files", file);
            });
            const fileLinks = await client.Configuration.Attachments.saveManyAttachments(areaIdentifier, formData);
            setAttachmentList(fileLinks);
        } else {
            const fileLinks = [];
            setAttachmentList(fileLinks);
