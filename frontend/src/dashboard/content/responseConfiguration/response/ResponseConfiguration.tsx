...
export const ResponseConfiguration = () => {
    ...
    const savePrologue = async () => {
        const _prologue_ = await client.Configuration.updatePrologue(areaIdentifier, prologue);
        setPrologue(_prologue_);
        return true;
    };

    const saveEpilogue = async () => {
        const _epilogue_ = await client.Configuration.updateEpilogue(areaIdentifier, epilogue);
    ...
    const tableSaver = async () => {
        ...
        const updatedStaticTables = await client.Configuration.Tables.Static.updateStaticTablesMetas(areaIdentifier, staticTables);
    ...
    const loadEstimateConfiguration = useCallback(async () => {
        setIsLoading(true);
        const { prologue, epilogue, staticTablesMetas } = await client.Configuration.getEstimateConfiguration(areaIdentifier);
