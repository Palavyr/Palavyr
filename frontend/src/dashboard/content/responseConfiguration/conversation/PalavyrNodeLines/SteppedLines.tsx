...
import { Selector, Anchor, LineStyles, ParsedAnchor } from "@Palavyr-Types";
...
export const SteppedLineTo: React.FC<SteppedLineToProps> = ({
    ...
    useEffect(() => {
        window.addEventListener("scroll", debounce(handle, 10));
        window.addEventListener("resize", debounce(handle, 10));
        return () => {
            window.removeEventListener("resize", debounce(handle, 10));
            window.removeEventListener("scroll", debounce(handle, 10));
        };
    }, [sized]);
