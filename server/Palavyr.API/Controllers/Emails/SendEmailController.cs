...
namespace Palavyr.API.Controllers.Emails
{
    ...
    public class SendEmailController : BaseController
    {
        ...
        private SESEmail
            Client { get; set; } // Startup.cs handles finding credentials from appsettings.json through GetAWSOptions

        private static ILogger<SendEmailController> _logger;

        public SendEmailController(ILogger<SendEmailController> logger, AccountsContext accountContext,
            ConvoContext convoContext, DashContext dashContext,
            IWebHostEnvironment env, IAmazonSimpleEmailService SES, IConfiguration config) : base(accountContext,
        ...
        [HttpPost("area/{areaId}/email/send")]
        public async Task<bool> SendEmail([FromHeader] string accountId, string areaId,
            [FromBody] EmailRequest emailRequest)
        {
            _logger.LogDebug("Attempting to send email from widget");

            var pdfGenerator = new PdfResponseGenerator(DashContext, AccountContext, ConvoContext, accountId, areaId,
                Request, _logger);
